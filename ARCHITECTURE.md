...
graph TD
    ...
    parallel["parallel/ — ThreadPool, LiveCurveFeed"]
...
`trade::Book` organizational trees. `vol/` and `xva/` are engine-internal
...
|-------|-----------|------|
| `ad/` | `Dual`, **`Rev<T>`** (`ad/reverse.hpp`) | Forward-mode AAD scalar (Eigen `AutoDiffScalar`). The engine is templated on `Scalar` so `double` drives the solve and `Dual` yields the analytic Jacobian from the *same* code. **`Rev<T>`** is the additive reverse-mode tape (record-then-reverse) that gives whole-gradient-in-one-pass; composed forward-over-reverse (`Rev<Dual>`) it yields the curve **gamma**/cross-gamma (`calibration/gamma.hpp`) — second-order risk without bumping. Gradient parity vs forward-mode 7.8e-16, gamma vs FD 1.4e-10. |
