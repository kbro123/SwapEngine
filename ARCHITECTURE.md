...
aggregates per `NettingSet`/CSA (collateral-OIS discounting). Still engine-only (no verb yet):
`market::Scenario`/`Market::clone` forks and `trade::Book` organizational trees. `vol/` and `xva/` are engine-internal
