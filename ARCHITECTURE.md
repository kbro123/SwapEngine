...
| `api/` | `BundleSession`, `run_json`, `bundle_from_json`/`bundle_to_json`, `flat_x0` | The public seam: a JSON object-graph contract + a stateful session. QuantLib-free — this is what the web binding wraps. Carries turns end-to-end: a curve's `"turns":[{start,end}]` overlay and the `"TurnJump"` instrument (`turn_curve`/`turn_index` + band); `flat_x0` seeds each δ at 0. |
