...
def main():
    ...
    lines = [
        ...
        "struct ProductConv {",
        "  std::string_view id, type, currency, calendar, bdc, frequency, discount_index, pair, base_currency, fx_reset_calendar;",
        "  int spot_lag = -1, payment_lag = -1;  // -1 = unset (the builders require it where it matters)",
        "  // xccy_mtm: exchange settlement lags (market 0) and the FX reset fixing lag (CARR: 2), -1 = unset.",
        "  int exchange_lag_initial = -1, exchange_lag_intermediate = -1, exchange_lag_final = -1, fx_reset_fixing_lag = -1;",
    ...
    for pid in sorted(products):
        ...
        lines.append("  {" + ", ".join([
            ...
            sv(p.get("fx_reset_calendar")),
            str(p.get("spot_lag", -1)), str(p.get("payment_lag", -1)),
            str(p.get("exchange_lag_initial", -1)), str(p.get("exchange_lag_intermediate", -1)),
            str(p.get("exchange_lag_final", -1)), str(p.get("fx_reset_fixing_lag", -1)),
