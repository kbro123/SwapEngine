...
| R7 | *(module deleted in E6.1, 2026-09-10 — no consumer)* CMS Simpson `pow` bloat + `std::function` (`cms_replication.hpp:26-76`) | cache `a,aN,aD` per node; template `vol_at` to inline SABR. **DONE (pow-cache):** `GFunctionStandard` now evals `a^N`,`a^delta` ONCE and derives every other power by mul/div; `derivs(x,g1,g2)` shares them across G'/G'' so the Simpson integrand issues 2 `pow`/node not ~16. Bit-close (oracle G' 1e-9 / G'' 1e-7 hold). `cms_replication_bench`: /1000 382→148µs (**2.6×**), G-deriv fan 36→11µs (3.2×). Still TODO: template `vol_at` to inline SABR (skipped — signature/compile-mem risk near the web TUs). |
| R8 | SABR vol recomputed per strike; ATM via full smile (`options.cpp:115`) | `sabr_node()` memoizes F/T part; closed-form ATM |
| R9 | *(module deleted in E6.1, 2026-09-10 — no consumer)* `strip_caplet_vols` O(n²) (`cap_stripping.hpp:87-95`) | carry prefix PV as running sum → O(n). **DONE:** dropped the per-bucket `whole` vector copy (inline target sum) and replaced the stripped-prefix re-sum with a running `fixed` accumulator (byte-identical caplet(v) evals). New `cap_stripping_bench`: 30y/4 174→139µs (1.25×), 40y/2 (denser) 409→287µs (1.43×) — denser curve benefits more, confirming the O(n²) prefix removal. Tests 147–149 green. |
