...
class Market {
 public:
  ...
  Market& add_curve(const std::string& name, const std::vector<curve::CurveModule>& modules,
    ...
    crv.set_forwards(x);
    curves_.insert_or_assign(name, std::move(crv));
    return *this;
  }
  ...
  const curve::ModularCurve<double>& curve(const std::string& name) const {
 ...
 private:
  ...
  std::map<std::string, curve::ModularCurve<double>> curves_;  // NAMED realized curves — the missing lookup
