...
| `asset_swap_oracle_test.cpp` | The `asset_swap` VERB end to end (run_json -> calibrated bundle -> float leg -> spread) vs QuantLib's `AssetSwap` legs written out as a `Swap` with the DB payment lag on the float coupons (`IborLeg::withPaymentLag`; `AssetSwap` itself has none, and at lag 0 the written-out `Swap` equals `AssetSwap(parSwap=true/false)` -- pinned), on identical DFs and QuantLib's own schedules: spread in all three price modes, annuity, curve dirty/clean, accrued, the proceeds identity (par / dirty), a leap-day maturity's float roll | 1e-10 (`tol::curve_rel`) |
| `fx_forward_spot_date_oracle_test.cpp` | FX forward OUTRIGHT (`QuoteKind::FxForward`) vs QuantLib core `FxSwapRateHelper::impliedQuote()` with the spot quoted for the SPOT DATE (O-X3 fx_spot_time), on identical discount factors (engine curves through `CurveTermStructure`) and QuantLib's own spot / delivery dates, T+2 and a US-Thanksgiving joint-calendar spot, 1W..1Y. Premise: QuantLib's spot date == the DB EURUSD spot. Negative controls: t_s = 0 (today's t = 0 reading) and t_s = T both miss. Reproduced pre-field (missed by 1.15e-4 / 2.88e-4 relative); measured with fx_spot_time: ~2e-16 | `tol::curve_rel` (residual `curve_rel / T`) |
...
| `matched_swap_multiccy_oracle_test.cpp` | The `swap_spread` MATCHED-MATURITY swap (derive::spread_swap) per currency vs QuantLib `MakeOIS` / `MakeVanillaSwap` built from the DB rows on QuantLib's own schedule and calendars (settlement days = the product spot_lag, termination = the bond maturity, Backward, EOM off): USD-SOFR T+2 over Labor Day, EUR-ESTR T+2 over Easter, GBP-SONIA T+0 on Columbus Day, AUD-AONIA T+1 over NSW Labour Day, AUD-BBSW-6M (semiannual, 20-day stub), EUR-EURIBOR-6M (annual 30E/360 vs 6M: two front stubs) and the month-end FS1 case. Spot and termination exact; every coupon's pay time, accrual and accrual dates; the par rate on our curve vs fairRate(). Controls per case: another currency's spot LAG (USD T+2 on GBP / AUD), the lag on another currency's CALENDAR, Forward stub side, Unadjusted termination; plus QuantLib dropping a roll date that adjusts onto spot. Out of scope: zero-coupon and day-frequency products, non-business value dates (O5, under research), two-curve swaps | par `curve_rel`, dates / accruals `literal` (measured on first run: see the commit) |
| `scenario_oracle_test.cpp` | The `scenario` and `scenario_grid` VERBS end to end on a DB-built 3-curve bundle (ESTR with a year-end turn, EURIBOR-6M on ESTR discounting, SOFR) and a book of 4 dated OIS/IRS from `swap_conv` → `build::par_swap`: every move's book `npv` / `npv_delta` and sampled DF / zero, and every grid cell's `npv` / `pnl`, vs QuantLib `OvernightIndexedSwap` / `VanillaSwap` on QuantLib schedules and indices, priced on (L1) our curve rebuilt at the verb's `base.x` + shift and (L2) `ZeroSpreadedTermStructure` over the base. A PARALLEL move on a SPREAD curve (EURIBOR-6M over ESTR) moves it once, as QuantLib reads it -- the pin that found the 2x double move fixed in b5f3a4d. Controls: SC1 -- a key ADDS onto the parallel (scenario vs the old replace rule, grid vs override), sign, turn δ untouched, wrong role. Out of scope: MtM xccy / FX moves, turn calibration | measured worst: NPV 2.6e-8 on 1e8 gross, DF 3.3e-16, zero 3.9e-15; locked at NPV 1e-6, DF/zero 1e-13 |
| `portfolio_verb_oracle_test.cpp` | The `portfolio`, `portfolio_risk`, `risk` and `transform` VERBS end to end on a hard square bundle (SOFR, ESTR, EURIBOR-6M as a spread over ESTR) and a book of TYPED TRADES booked through `book.trades` (value_date, curve_roles, CSA): per-trade and book NPV and PV01 vs QuantLib `OvernightIndexedSwap` / `VanillaSwap` on QuantLib's own schedules and signs (PV01 as +1bp on every curve once, written out test-side); `curve_grad`, the risk operator and the ladder vs QuantLib's central-difference Jacobian and its inverse; `transform` vs J_src·J_this⁻¹ (reg OFF only). Two trades are booked to WEEKEND maturities (T6 Sunday, T7 Saturday) and must roll like QuantLib's termination date. Controls: pay direction, discount role (two-sided), fixed day count, QuantLib with an Unadjusted termination on T6/T7 must miss (WK1, d52038c), a SEASONED trade must be refused (this control found SW1: a stateless session forecast the realized part), par feedback, the ladder sees discounting, the transform sees the forecast role, the xccy block is zero | measured: NPV 5.5e-16/notional (book 4.2e-9 on 9e7), PV01 3.7e-10 rel, gradient 5.1e-10, M 2.1e-9, ladder 3.3e-8, transform 1.2e-8 (cond 475); weekend control misses by 2.8e-5/notional (gap 1e-5); locked NPV `curve_rel`·N, PV01/gradient `jacobian_rel`, M/ladder/transform 3e-6 |
