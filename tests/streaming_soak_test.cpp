...
//   Light TENSION (the API default)  factor 82 / 22 @ 1,400 us     per-row 59 / 48 @ 5,000 us     (after the operator fix)
//   Strong tension                   factor  0 /  0 @    36 us     per-row  6 /  4 @ 1,600 us
//   Light SECOND-DIFFERENCE          factor  0 /  0 @    31 us     per-row  0 /  0 @   268 us     zigzag <= 3 bp everywhere
...
//   SMOOTHED (Light second-difference over every curve) -- the recipe that streams EVERY rung: ZERO failed ticks on both
//     walks. That is the invariant.
//   SHIPPED PRESET (Light tension, what compile_reg_spec hands a banded / under-determined bundle) -- its measured counts
//     on the factor walk, so the default's cost on a value-dependent region is on record and may only DECREASE.
...
enum class Recipe { Smoothed, ShippedPreset, Unsmoothed };
const char* to_string(Recipe r) {
  switch (r) {
    case Recipe::Smoothed: return "SMOOTHED (Light second-difference over every curve)";
    case Recipe::ShippedPreset: return "SHIPPED PRESET (Light tension, the API's default for a banded bundle)";
...
cal::RegSpec reg_for(Recipe r, const Shape& s) {
  ...
  switch (r) {
    ...
    case Recipe::ShippedPreset: return cal::smoothing_preset(cal::Smoothing::Light, nc);
    case Recipe::Smoothed: {
      cal::RegSpec reg = cal::smoothing_preset(cal::Smoothing::Light, nc, /*tension=*/false);
...
// PINS: failed ticks per 400-tick walk, measured 2026-09-21 (seed 20260921) after the regularize.hpp fix. Every SMOOTHED row is
// pinned at ZERO -- that is the invariant. The SHIPPED PRESET and UNSMOOTHED rows carry the value-dependent rungs' measured
// counts on the factor walk (shipped: mixed_scheme 82, desk_mixed 22; unsmoothed: 49, 85) plus ~25 % slack; a linear rung is
// zero under every recipe. They may only DECREASE: lower a pin when you fix the cause, never raise one.
struct Pin { const char* name; long shipped_factor, unsmoothed_factor; };
const Pin kPins[] = {{"mixed_scheme", 103, 62}, {"desk_mixed", 28, 108}};
long pin_for(const std::string& name, Recipe r, long n) {
  if (r == Recipe::Smoothed) return 0;
  for (const Pin& p : kPins)
    if (name == p.name) return ((r == Recipe::ShippedPreset ? p.shipped_factor : p.unsmoothed_factor) * n + 399) / 400;
  return 0;
}
...
void run_walk(WalkKind kind, Recipe recipe) {
  ...
  for (const Shape& s : ladder()) {
    ...
    EXPECT_LE(o.failed, pin_for(s.name, recipe, n)) << s.name << " (" << swaps::shapes::to_string(kind) << " walk, " << to_string(recipe)
                                                    << "): failed ticks above the pin";
    // 0.01 bp between the delivered curve's quotes and the cold solve's. A value-dependent rung under a recipe that still fails
    // ticks is judged at 0.1 bp: there the fallback LM itself stalls on the kinked landscape (0.06 bp measured, ShippedPreset).
    const double dq_tol = (s.expect_compiled || recipe == Recipe::Smoothed) ? 1e-6 : 1e-5;
...
TEST(StreamingSoak, FactorWalkUnderTheShippedPresetIsOnRecord) { run_walk(WalkKind::Factor, Recipe::ShippedPreset); }
