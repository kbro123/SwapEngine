...
| C9 | `calibration/streaming.hpp` + `api/bundle_api.cpp` streamed tick, on a bundle with a VALUE-DEPENDENT region | frozen-Newton: freeze J, refresh only on staleness | **The premise does not hold there.** desk_mixed refreshes its Jacobian on EVERY tick (2000 of 2000 driven ticks; desk and mixed_scheme refresh zero), so its "streamed" tick is really a per-tick J rebuild — 1.83 ms against desk's 45 us. Still 43x better than the cold LM it replaced (214.7 ms), but it is not streaming in the sense the rest of the ladder means. Worse, the tick INTERMITTENTLY fails: `desk_mixed stream tick did not converge: refresh cap hit`, seen once in a full `verify.sh` and not reproduced in four subsequent runs of the same command — latent before ad::kPooledMaxW 48→64, which only made it reachable by tripling the iterations 0.5 s buys | the bench itself (a metric must never time a failing tick) | **OPEN, and the top of the list**: a rare non-converging tick makes the perf gate flaky, which is more corrosive than a gate that fails honestly. Find the cap exhaustion; decide whether a value-dependent bundle should re-anchor rather than refresh |
