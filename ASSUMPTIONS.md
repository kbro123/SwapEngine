...
| C9 | `calibration/streaming.hpp` `breakeven_steps_` | the adaptive stall's refresh-vs-step ratio is a fair cost model | **DIAGNOSED 2026-09-12 — a solver control parameter derived from WALL-CLOCK measurements.** `breakeven_steps_ = clamp(refresh_ns / step_ns, 2, 64)` is measured once per calibrator construction by timing one Jacobian+factorisation against one residual evaluation. Under CPU load that ratio can land near its floor of 2, so the stall refreshes after ~2 frozen steps; on a bundle whose frozen operator never contracts (a value-dependent region — desk_mixed refreshes J on every one of 20,000 ticks where desk and mixed_scheme refresh none) six refreshes are consumed in ~14 steps and the tick fails `RefreshCap`. Failing signature is exactly `refreshes=6 steps=14` (2.3 steps/refresh). REPRODUCTION: drive desk_mixed's streamed tick with the machine loaded (8+ spinners) — 5/6 bench runs and the raw calibrator fail at tick 0; quiet, 0/20 runs and 0/20,000 ticks. Ruled out: the background Jacobian worker (off by default, the session does not enable it), a regulariser (RegSpec defaults to off), and any per-tick clock use (the clock is read only at construction). | `tests/streaming_contract_test.cpp` `TheAdaptiveStallsBreakEvenIsDeterministicUnlessTheCallerAsksForTheTiming` | **RESOLVED 2026-09-21** (independent review, finding 2): the timing is now OPT-IN (`Options::measure_breakeven`) and the default is the deterministic constant `kBreakevenCap = 64`, so identical inputs give identical control flow and the load-induced floor-of-2 failure cannot occur. The constant rather than a structural cost model because the measurement SATURATED at the cap on every desk shape probed (8x26 chain and 3x12: 64.00 on every repeat; a single 26-knot curve: 30.6–33.4, and a refresh there is cheap), so the model would only be fitting the one cheap case. The construction-time warm-up pass (one residual + one Jacobian + factorisation) is KEPT un-timed — it sizes the per-tick scratch, which is what makes the tick allocation-free (`hotpath_census_test.cpp` pins those counts, and dropping it moved two scenarios to 11 and 15 allocations). Still open, separately: cap exhaustion fails the tick rather than falling back to an LM solve |
| E3 | `build/observations.hpp` `daily_periods` | an averaged overnight window's fixings are the business days INSIDE it | a window opening or closing on a NON-BUSINESS DAY lost that accrual while `tau_index` kept the whole window: Saturday-start August 2026 priced at 29/31 of the correct rate (−25 bp on a flat 4 %), Sunday-start November at 29/30, and a Sunday CLOSE truncated the trailing fixing to a 2-day rate. A CME 30-day FF/SOFR future references the CALENDAR month, so roughly a third of contract months open this way | found 2026-09-10 by `calibration_oracle_test` (QuantLib's instruments, our solver) on its first run; fixed as `FixingPeriod` — the fixing keeps its own overnight window, the observation window clips only the ACCRUAL; pinned by `build_instruments_test::AnAveragedWindowEarnsEveryDayWhateverDayItOpensOn` against the H.15 day-by-day definition |
...
| T10 | `api/bundle_api.cpp` `stamp_streamed` (stream_update path) | `result().rms_residual` after a converged `stream_update` is the streamer's final corrector residual (`StreamingCalibrator::last_residual`, the state one sub-`step_tol` step before `x()`), not a re-evaluation at `x()` | 4.3e-8 reported vs 2.7e-9 re-evaluated on the hard `ApiState` bundle (~1e-3 bp; predictive convergence stops the corrector one step early) — bounded by O(‖J‖·step_tol). `resolve`/`recalibrate`/`rebind` re-evaluate exactly (their callers pin 1e-10) | `api_state_test.cpp` `StreamUpdateReportsItselfInResultLikeResolveDoes` | accepted 2026-09-21: an exact rms would put a residual pass on the gated tick; the report exists at all because a converged `stream_update` used to leave `result()` at the previous LM solve |
...
| D7 | `hybrid_residual.hpp` `curves_are_noncacheable`, `bundle_api.cpp has_nonlinear_` | `Scheme::MonotoneCubic` enumerated by name in two places | — | **CLOSED 2026-09-12**: both now read `curve::scheme_is_linear` (the one definition), and neither decides anything wholesale any more — the router partitions per row (1956b24) and `needs_recalibrate()` is retired (always false; a mixed bundle streams, measured 38x/43x faster than the cold solve it was forced into). One whole-bundle use remains, in a different engine: `portfolio/compiled_multi.hpp` still switches the WHOLE compiled book on `curves_are_noncacheable` — same over-approximation, not yet addressed |
...
| D17 | `calibration/regularize.hpp` `curve_tension_stiffness` | a VALUE-DEPENDENT region (MonotoneCubic) contributes a DISCRETE tension energy -- the composite-trapezoid bending ½(h₀+h₁)(f″ᵢ)² and membrane hⱼ(f′ⱼ)² of its knot values plus the C0 join point -- instead of the shape-function quadrature, which is defined only for a linear map (it evaluates the curve on unit knot vectors; the Hyman filter clamps such a spike, and the resulting stiffness penalised the wrong directions: under `Smoothing::Strong` the streaming soak measured desk_mixed DRIVEN into a 149 bp zigzag, 382/400 failed ticks). Same units as the continuous energy, so one weight serves both kinds of region. NB the tension presets are ~1/h³ weaker than the second-difference presets at knot spacing h (≈1/125 at 5y): Light tension still leaves desk_mixed at 22 failed ticks / 400 on the factor walk where Light second-difference gives 0 (`streaming_soak_test.cpp`, its header table) | pinned by `tension_regularizer_test.cpp` `AValueDependentRegionContributesItsDiscreteEnergy` (affine → zero bending, exact membrane across the join; zigzag == hand-computed discrete energy) and the soak gate | accepted 2026-09-21 |
...
| E2 | `build/observations.hpp` averaged / lookback / lockout observations | **CORRECTED 2026-09-10 (item 17).** A bracket contributes `DF(s)/DF(e) − 1` — the window's GROWTH — which becomes a rate only when divided by that window's length IN THE INDEX'S DAY COUNT. All four builders divided by its CURVE-TIME length, so every averaged overnight leg and every lookback/lockout coupon was (index dc)/(curve dc) too high: exactly 365/360 = +1.389 % for an ACT/360 index. Flat 4 % curve, 3M FF window: 400.04 bp where the daily EFFR fixings average 394.56 bp. | fixed (`obs_weight`, one definition); pinned by `BuildInstruments.FloatLegHonoursTheProductsCompoundingConvention` against the H.15 average computed from the curve, and by the compounded-vs-averaged Jensen gap (1.93 bp measured vs r²τ/2 = 2.02 bp predicted). The QuantLib oracles never saw it — their fixtures compute the weight correctly — and `ObservationWindowsMatchPython` had pinned the wrong number as C++/Python parity. The web compiler still produces it: TASKS-API §A0.5. |
| E1 | the JSON **text** seam (`run_json`, the C ABI, any `serialize`→`parse`) | Boost.JSON's serialize→parse is NOT bit-exact: measured 2026-09-10 over 200k random rates, **9.5 % of doubles come back 1 ULP different** (worst 2.2e-16 relative; its printer emits a shortest-ish decimal its correctly-rounded parser does not map back). So a request that reaches the engine as TEXT hands the solver a market differing from the caller's in the last bit, and two seams differ by ~5e-16 on x. | measured + pinned (`BundleApi.ParseOnceEntryMatchesTheStringSeamAndEveryVerbKeyRoutes`). The parsed-OBJECT entry (E6.3) has no round trip and is the exact one; the compile+sample re-entry stopped serialising. Value-level codecs (`bundle_to_json`/`bundle_from_json`, no text) are exact. |
